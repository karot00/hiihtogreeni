...
Phase 10 automated validation took about 10 minutes and cost 2,8 € in AI usage, using GPT-5.6 Sol in Kilo Code. The remaining step is the manual browser review and the pre-cutover host and rollback checks.

## Where the Project Is Now - Phase 10 manual QA fixes applied

The manual browser review against the staging URL has now been done, and the issues it surfaced were fixed in code and pushed to GitHub. This is the hands-on part of Phase 10 that automated tests cannot catch: how the site actually behaves for a person holding a phone.

The most important fix was the mobile navigation. On small screens the menu button opened a panel that was effectively invisible: it appeared only inside the top bar, showing one item at a time. The cause was subtle and worth remembering. The page header uses a backdrop blur for the frosted look, and in CSS a backdrop filter makes that element the containing block for any fixed-position descendant. The menu panel was rendered inside that header, so "fixed" was measured against the 64-pixel header box rather than the screen. The fix was to render the panel through a portal straight to the document body, so it now covers the full screen below the header and scrolls naturally. This is the kind of bug that passes every automated test and only shows itself in a real device.

Several visibility and clarity improvements followed. On the dark hero photograph and on the dark green closing call-to-action band, the secondary buttons used dark-green text on a dark background and were nearly invisible. We added a light button variant (white background, dark-green text) for exactly these dark surfaces, while keeping the primary green button where it sits on light content. The hero now reads clearly: "Tutustu mökkiin" stays green, "Ota yhteyttä" is white, and both closing-band actions are white.

The green-fee journey was simplified to match how the business actually works. The green-fee button on the homepage and the green-fee call-to-action on the rates page now link directly to `greenfee.levifinland.fi` and open in a new tab, and the separate "Green fee -varaus: greenfee.levifinland.fi" text line on the rates page was removed as redundant. Similarly, the "Ready to help?" closing section on the contact pages, which repeated the phone and cabin links already present in the header and footer, was removed so the contact page stays focused on the form.

The contact form was the last piece of unfinished localization. The English form was still showing Finnish validation messages, a Finnish submit button ("Lähetä viesti"), and a Finnish success message, even though the recipient received an English email. We moved all the form's runtime strings (required-field and email errors, rate-limit and error messages, the submit and sending labels, and the success message) into the same per-language content modules used everywhere else, so the Finnish and English forms are now fully in their own language. While there, we aligned the phone field: the browser was marking it required while the server already treated it as optional, so we made phone optional on both sides and labeled it "(valinnainen)" / "(optional)".

One deliberate removal: the "Siirry sisältöön" skip link was taken out of the shared shell at the owner's request. It is normally a WCAG 2.2 AA aid that lets keyboard and screen-reader users jump past the navigation to the main content, so removing it does leave that specific checklist item open; it can be restored in one line later if accessibility certification becomes a requirement.
