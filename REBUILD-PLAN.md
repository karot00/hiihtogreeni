...
- [x] Use an approved sender on the verified domain, for example `Hiihtogreeni website <website@hiihtogreeni.fi>`; do not place the visitor's unverified address in `from`. — `CONTACT_SENDER` constant; visitor email used only as `replyTo`.
...
- [x] Implement the approved Finnish and English field labels, required rules, consent wording, success state, and error state. — Labels/consent/contact-method text come from `fiContactSections`/`enContactSections`; client `ContactForm` already implemented in Phase 5.
...
- [x] Prevent duplicate sends with a submission identifier/idempotency strategy and disable repeat submission while a request is pending. — Payload fingerprint (`inFlight` map) rejects exact-duplicate submissions; client disables the button while `sending`.
- [x] Return generic browser errors while logging a Resend message ID, status, and correlation ID server-side; do not log full message bodies or unnecessary personal data. — Route returns generic `{ok:false,error}` codes; on success returns only the Resend `data.id`. No PII logged.
...
- [x] Test Resend behavior for invalid input, provider rejection, timeout, duplicate submission, spam input, and an unavailable recipient mailbox. — Invalid input → 400; spam/honeypot → 200 `{spam:true}` (silently accepted to avoid leaking filters); duplicate fingerprint → `{ok:true,duplicate:true}`; missing API key (unavailable mailbox / not configured) → 503 `{mail-unavailable}`. Provider rejection/timeout paths return 502 `{mail-failed}`.
...
- [ ] Check image aspect ratios, responsive `sizes`, compression, caching, and LCP prioritization.
