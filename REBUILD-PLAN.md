...
- [x] Integrate the Google tag as a client component analogous to levifinland2026's `UmamiScript` (`packages/ui/src/components/umami-script.tsx`) — e.g. `src/components/GoogleAnalytics.tsx` using `next/script` with `strategy="afterInteractive"` — gated by the Phase 13 `isCategoryAllowed(consent, "analytics")` helper and mounted once inside `SiteShell.tsx` alongside `CookieConsent`. — **Completed 2026-07-27:** `GoogleAnalytics` mounted once in `SiteShell` inside `CookieConsentProvider`; gtag.js loader is injected only after the analytics category is granted.
