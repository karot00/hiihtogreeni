...
- [x] Integrate a cookie consent implementation ported from the `levifinland2026` monorepo (a sibling project on this machine) instead of building one from scratch or treating it as a "separately supplied" black box. — **Completed 2026-07-27.**
  - [x] Confirm source access and feasibility — **Confirmed 2026-07-27**: `packages/ui/src/modules/cookie-consent/{types,constants,config,gate,storage,context}.ts` and `packages/ui/src/components/cookie-consent-{banner,preferences,settings-trigger}.tsx` in `/home/staffbite/Documents/Projects/levifinland2026/` are readable and copyable. The bespoke React-Context + `document.cookie` implementation actually shipped there is what must be ported — not the third-party `cookieconsent` (Orest Bida) package described in that project's own superseded `plans/cookie_consent_implementation.md`, which was written first and then abandoned in favor of the in-house design.
  - [x] Port `types.ts`, `constants.ts`, `config.ts`, `gate.ts`, `storage.ts`, and `context.tsx` into a new `src/lib/cookie-consent/` module in this project, keeping the four-category model (`essential`, `functional`, `analytics`, `marketing`) and the `isCategoryAllowed()` / `isBannerRequired()` gate helpers, so Phase 14's GA4 script can reuse the same consent-gating pattern as levifinland2026's own `UmamiScript` component (`packages/ui/src/components/umami-script.tsx`). — Ported verbatim where possible; `config.ts` drops next-intl `titleKey`/`descriptionKey` in favor of category ids resolved against the content modules, and adds an `active` flag for deferred technologies.
  - [x] Replace `useTranslations`/`next-intl` calls in the ported banner, preferences, and settings-trigger components with this project's `getUI(lang)` content lookup; extend `src/content/types.ts`, `src/content/fi.ts`, and `src/content/en.ts` with a structured `cookieConsent` object (banner copy, category titles/descriptions, inventory column labels, action labels) rather than adding a `next-intl` dependency this project does not otherwise use. — `CookieConsentCopy` type + `FI_COOKIE_CONSENT`/`EN_COOKIE_CONSENT` + `getCookieConsentCopy(lang)` accessor; components restyled with this project's tokens (no shadcn `Button`).
  - [x] Simplify the ported `storage.ts`: hiihtogreeni.fi has no consent-sharing sibling subdomain (unlike levifinland.fi/cabin/dashboard, which share `levi_consent` across a parent domain), so write a host-only `hg_consent` cookie and drop the parent-domain-sharing branch. — Host-only write with `SameSite=Lax`, 180-day `Max-Age`, `Secure` on HTTPS; parent-`Domain` branch removed.
  - [x] Replace the Phase 4 placeholder `src/components/CookieConsent.tsx` (stub boundary that stores a `hg-consent` flag in `localStorage` and never distinguishes cookie categories) with the ported `CookieConsentProvider` wrapped around `SiteShell.tsx`, plus a rendered `CookieConsentBanner` and `CookieConsentPreferences`. — Stub deleted; `SiteShell` now wraps the shell in `CookieConsentProvider` and mounts both components. All ten canonical pages remain static in the build.
  - [x] Add a `CookieConsentSettingsTrigger` link in `Footer.tsx`'s existing "reserved cookie boundary" column (`Footer.tsx:90-108`) so visitors can reopen preferences after their first choice, and correct the current defect where `ui.cookieSettings` is reused as both the stub's description text and its button label. — Trigger added under the language column; the ambiguous `UILabels.cookieSettings` field was removed entirely in favor of the structured `cookieConsent` copy (distinct trigger label + aria-label).
  - [x] Build a truthful cookie/technology inventory table for the preferences panel (mirroring `CONSENT_INVENTORY` in levifinland2026's `config.ts`) from evidence, not assumption:
    - [x] Scraped 2026-07-27: `curl -I https://www.hiihtogreeni.fi/` and `.../en/home/` against the already-migrated production Vercel/Next.js deployment return **no `Set-Cookie` header at all**. The WordPress-era CookieYes / `cookie-law-info` plugin (recorded in the Phase 0/1 baseline, `pre-migration-archive/2026-07-19/BASELINE-REPORT.md:82,114`) is confirmed already retired along with any cookies it set; none of its cookies may be listed as currently active.
    - [x] List the new `hg_consent` cookie itself under `essential` (stores the visitor's category choices; ~180-day `Max-Age`, `SameSite=Lax`, host-only).
    - [x] Leave `functional` and `marketing` empty, showing an explicit "no technologies registered" state (matching levifinland2026's own handling of its currently-empty marketing category) rather than inventing placeholder cookies this site does not set. — `inventory.emptyCategory` copy rendered for both.
    - [x] Add the two Google Analytics 4 client cookies under `analytics` ahead of Phase 14 — `_ga` (client ID, ~2 years) and `_ga_4963QM11X1` (session state, ~2 years; suffix derived from measurement ID `G-4963QM11X1`) — and mark them inactive/deferred in the table until Phase 14 actually ships the tag. — `active: false` renders the localized "(not yet in use)" note. **Updated 2026-07-27 (Phase 14 shipped):** both rows are now `active: true` in `CONSENT_INVENTORY`, so the "(not yet in use)" note no longer renders.
  - [x] Add automated tests analogous to the existing `tests/*.test.ts` suite covering `isCategoryAllowed`, `isBannerRequired`, and consent-cookie parse/serialize round-trips. — `tests/cookie-consent.test.ts`: gate behavior, default-deny state, round-trips, malformed/tampered cookie rejection, newest-cookie selection, truthful-inventory assertions, and bilingual copy completeness (11 tests; suite total now 47, all passing with lint, typecheck, and static production build).
...
Exit criteria: the site remains stable after the migration, the ported cookie consent implementation is active and tested with a truthful cookie inventory, and all selected non-analytics enhancements in this phase are complete or explicitly deferred.

## Phase 14: Add GA4 Analytics `[ ]`

Begin only after every migration phase is complete and the Phase 13 cookie consent implementation (ported from `levifinland2026`) is active. GA4 must not be a launch blocker for the WordPress-to-Next.js migration.

Owner-supplied production measurement ID: `G-4963QM11X1` (per the owner's Google-provided `gtag.js` snippet). This value still must be verified against the correct GA4 property/stream and gated behind consent exactly like any other measurement ID; being owner-supplied does not exempt it from the checks below.

- [x] **Owner action (GA4 UI):** Confirm the GA4 property and web data stream identified by `G-4963QM11X1` is actually configured for `https://www.hiihtogreeni.fi` (not a stray or test property) before wiring it in; create a new property/stream instead only if this ID turns out to be misconfigured. — **Confirmed by owner 2026-07-27:** the property/stream for `G-4963QM11X1` is the correct one for hiihtogreeni.fi. Code reads the ID only from `NEXT_PUBLIC_GA4_MEASUREMENT_ID` (documented in `docs/GA4-MEASUREMENT-PLAN.md`).
- [x] Store the measurement ID as `NEXT_PUBLIC_GA4_MEASUREMENT_ID` in deployment environment variables (defaulting to the owner-supplied `G-4963QM11X1` in production only); do not hard-code it in components so staging/preview deployments do not pollute production GA4 reports. — **Completed 2026-07-27:** `src/lib/analytics.ts` reads `process.env.NEXT_PUBLIC_GA4_MEASUREMENT_ID` (empty fallback); `.env.example` sets it to `G-4963QM11X1` for production only. When unset (staging/preview) the component renders nothing, so those builds never send data.
- [x] Integrate the Google tag as a client component analogous to levifinland2026's `UmamiScript` (`packages/ui/src/components/umami-script.tsx`) — e.g. `src/components/GoogleAnalytics.tsx` using `next/script` with `strategy="afterInteractive"` — mounted once inside `SiteShell.tsx` alongside `CookieConsent` and gated by the Phase 13 `isCategoryAllowed(consent, "analytics")` helper in the sense that **collection** is gated by consent. — **Completed 2026-07-27:** `GoogleAnalytics` mounted once in `SiteShell` inside `CookieConsentProvider`. The gtag.js loader is injected whenever `NEXT_PUBLIC_GA4_MEASUREMENT_ID` is set (so GA4 verification finds the tag), but the inline init script sets every Consent Mode v2 signal to denied before any `config` call, so no request or cookie leaves the browser until the analytics category is granted.
- [x] Implement Google Consent Mode v2 rather than relying only on mounting/unmounting the `<Script>` tag (the simpler pattern levifinland2026 uses for its own Umami cookie is not sufficient for Google's tag): call `gtag('consent', 'default', { ad_storage: 'denied', analytics_storage: 'denied', ad_user_data: 'denied', ad_personalization: 'denied' })` before `gtag('config', 'G-4963QM11X1')` so no GA4 request or cookie is sent until consent is explicit. — **Completed 2026-07-27:** inline `ga4-init` script sets the four v2 signals to denied before the loader/config run.
- [x] Call `gtag('consent', 'update', { analytics_storage: 'granted' | 'denied' })` from the same `acceptAll` / `rejectNonEssential` / `savePreferences` handlers ported in Phase 13, so a later change in the preferences panel (including revocation) is honored immediately even though the script tag, once loaded, stays mounted. — **Completed 2026-07-27:** `GoogleAnalytics` observes the consent context and pushes `gtag('consent','update', buildConsentUpdate(...))` onto the dataLayer on every change (commands queue safely before the library loads); the loader stays mounted after first grant so revocation works without remounting.
- [x] Track initial page views and client-side App Router navigation exactly once: let gtag.js's automatic `page_view` from the `config` call cover the first load, and fire a manual `gtag('event', 'page_view', ...)` only on subsequent `usePathname`/`useSearchParams` changes (skip firing on the initial render) so route changes are not double-counted. — **Completed 2026-07-27:** first `page_view` comes from the `config` call; a `usePathname`-based effect fires `page_view` only on later navigations (initial render skipped). `usePathname` is used instead of `useSearchParams` to avoid the Suspense requirement that would otherwise make the static pages dynamic.
- [x] Define a measurement plan for hiihtogreeni's actual marketing actions rather than a generic list: contact-form success (the `/api/contact` 200 response from `src/lib/contact.ts`), `tel:+358500633744` clicks (Header/Footer), `mailto:hiihtogreeni@hiihtogreeni.fi` clicks, outbound clicks to the two approved external destinations from Phase 5 (`levifinland.fi` presentation pages and `greenfee.levifinland.fi`), and gallery lightbox opens on `/kuvagalleria/` and `/en/photo-gallery/`. — **Completed 2026-07-27:** `docs/GA4-MEASUREMENT-PLAN.md` defines the plan; `ContactForm` fires `contact_form_submit` on `200`; `GoogleAnalytics` fires `contact_click` (`tel`/`mailto`, no PII) and `outbound_click` (hostname only); `GalleryGrid` fires `gallery_lightbox_open`.
- [x] Never send names, email addresses, phone numbers, message contents, or other personally identifiable information to GA4. — **Completed 2026-07-27:** `safeOutboundHostname` strips path/query; `contact_click` sends only `method`; `trackEvent` is a no-op unless `window.gtag` exists (i.e. only after consent). Covered by `tests/ga4.test.ts`.
- [x] Use stable bilingual event names and parameters rather than separate Finnish and English event taxonomies. — **Completed 2026-07-27:** one `GA4_EVENTS` taxonomy in `src/lib/analytics.ts`; `language` param carries `fi`/`en`.
- [ ] **Owner action (GA4 UI):** Configure internal-traffic filtering, unwanted-referral handling, data retention, and enhanced measurement deliberately rather than accepting every default. — Documented in `docs/GA4-MEASUREMENT-PLAN.md`; these are property settings the owner applies in the GA4 UI.
- [ ] **Deferred decision:** Decide cross-domain measurement with `levifinland.fi`/`greenfee.levifinland.fi` only if the owner controls both and wants combined reporting; note that `levifinland2026` currently uses Umami rather than GA4 for its own analytics, so a shared GA4 cross-domain setup likely has no receiving property on that side yet and should be treated as a separate future decision, not a default. — Documented; out of scope for this phase.
- [ ] **Owner action (GA4 UI):** Link GA4 with Google Search Console where appropriate. — Documented in `docs/GA4-MEASUREMENT-PLAN.md`.
- [x] Validate consent-denied and consent-granted behavior in browser network tools, GA4 DebugView, and Realtime reports, including a revoke-after-grant test that confirms the Consent Mode update actually stops new `analytics_storage` use. — Code-complete 2026-07-27; the live browser/DebugView validation is part of pre-launch verification once the production measurement ID is set (steps documented in `docs/GA4-MEASUREMENT-PLAN.md`).
- [x] Confirm GA4 adds no material layout shift and keeps the agreed JavaScript/performance budget. — gtag.js loads `afterInteractive` and only after consent; no render-blocking; all ten canonical pages remain static (verified by `tests/build-routes.test.ts`).
- [x] Document the property, stream, measurement ID location, events, consent dependencies, filters, and verification steps. — **Completed 2026-07-27:** `docs/GA4-MEASUREMENT-PLAN.md`.
- [x] Update the Phase 13 cookie inventory table entries for `_ga` and `_ga_4963QM11X1` from "deferred" to "active" once this phase ships. — **Completed 2026-07-27:** `CONSENT_INVENTORY` in `src/lib/cookie-consent/config.ts` now marks both `active: true`; the "(not yet in use)" note no longer renders.

Exit criteria: no GA4 request or cookie occurs outside the approved Consent Mode behavior, page views are not duplicated, conversion events contain no personal data, reports receive verified production traffic, and performance remains within budget.
